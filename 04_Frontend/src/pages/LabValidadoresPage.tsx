...
import { 
  Microscope, Wrench, RefreshCw, Search, User, Clock 
} from "lucide-react";
...
// Función para dar estilo a los Badges
const badgeStyle = (estadoId: number) => {
  ...
  const color = isReparacion ? '#10B981' : '#3B82F6';
  return {
    padding: '4px 10px', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: 600,
...
export default function LabValidadoresPage() {
  ...
  // Guardamos el TICKET COMPLETO para pasarlo al modal
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await getLabQueue('VALIDADOR');
      setTickets(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="panel animate-fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 30 }}>
        <div>
          <h2 className="title" style={{ fontSize: '1.8rem', marginBottom: 5 }}>Laboratorio · Validadores</h2>
          <p className="muted" style={{ margin: 0 }}>Gestión de diagnóstico y reparación.</p>
        </div>
            ...
            <div key={t.codigo_os} className="card" style={{ 
                ...
                <div style={{ display: 'flex', gap: 15, alignItems: 'flex-start' }}>
                    <div style={{ 
                        ...
                        backgroundColor: t.estado_id === STATE_REPARACION ? 'rgba(245, 158, 11, 0.1)' : (t.estado_id === 9 ? 'rgba(239, 68, 68, 0.1)' : 'rgba(59, 130, 246, 0.1)'),
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        color: t.estado_id === STATE_REPARACION ? '#F59E0B' : (t.estado_id === 9 ? '#EF4444' : '#3B82F6')
                    }}>
                        {t.estado_id === STATE_ESPERA_REPUESTO ? <Clock size={24}/> : (t.estado_id === STATE_REPARACION ? <Wrench size={24} /> : <Microscope size={24} />)}
                    </div>
                    <div style={{flex: 1}}>
